/* ODIUM Marvel Rivals — dual-state roster artwork v9.1 */
(()=>{
const VER='9.1';
...
function upgrade(card){if(!card)return;addCredit(card);if(card.dataset.hoverArtV9===VER&&card.querySelector(':scope > .mr-hero-art-base')&&card.querySelector(':scope > .mr-hero-art-hover'))return;const name=card.dataset.name||card.querySelector('.char-name')?.textContent?.trim();if(!name)return;card.dataset.hoverArtV9=VER;
...
function boot(){scan();const g=document.querySelector('#characterGrid');if(g)new MutationObserver(()=>requestAnimationFrame(scan)).observe(g,{childList:true,subtree:true});setTimeout(scan,250);setTimeout(scan,850);setTimeout(scan,1800);}
