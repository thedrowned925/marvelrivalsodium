/* ODIUM Marvel Rivals — robust roster artwork v11.1 */
(()=>{
const VER='11.1';
...
async function upgrade(card){if(!card)return;const name=card.dataset.name||card.querySelector('.char-name')?.textContent?.trim();if(!name)return;addCredit(card);if(card.dataset.artV11===VER&&card.querySelector(':scope > .mr-hero-art-base')&&card.querySelector(':scope > .mr-hero-art-hover'))return;card.dataset.artV11=VER;card.style.setProperty('--art-focus',FOCUS[name]||'50% 1%');card.querySelectorAll(':scope > .mr-hero-art').forEach(n=>n.remove());
 ...
 const bp=resolveUrl(baseCandidates(name)),hp=resolveUrl(hoverCandidates(name));
 const hoverUrl=await hp;
 if(hoverUrl){hover.src=hoverUrl;card.dataset.hoverArt='ready';}
 const quickBase=await Promise.race([bp,new Promise(r=>setTimeout(()=>r(null),700))]);
 if(quickBase){base.src=quickBase;card.dataset.baseArt='ready';}
 else if(hoverUrl){base.src=hoverUrl;card.dataset.baseArt='hover-fallback';card.dataset.baseContain='true';}
 const finalBase=await bp;
 if(finalBase&&finalBase!==quickBase){base.src=finalBase;card.dataset.baseArt='ready';delete card.dataset.baseContain;}
 if(!hoverUrl&&finalBase){hover.src=finalBase;card.dataset.hoverArt='base-fallback';}
 if(!hoverUrl&&!finalBase)card.dataset.artMissing='true';
}
